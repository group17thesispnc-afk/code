...
# Thesis display target resolution. The physical monitor should already be
# configured to 1280x720; the GUI layout uses this fixed design resolution so
# both roadside stations render consistently.
DISPLAY_WIDTH: int = 1280
DISPLAY_HEIGHT: int = 720

# Optional runtime configuration loaded from disk / env for field calibration.
CALIBRATION_ENV_VAR: str = "SMART_ROAD_CALIBRATION_FILE"
MODEL_PATH_ENV_VAR: str = "SMART_ROAD_MODEL_DIR"
CALIBRATION_FILE_PATH: str = os.path.join(
    os.path.dirname(__file__),
    "smart_road_alert_calibration.json",
)
...
class DashboardGUI:
    ...
    def update_status(self, signal_text: str, emergency: bool = False) -> None:
        ...
        # Hard-disable animations for performance/stability on the Pi.
...
class SmartRoadAlertHost:
    ...
    def __init__(self) -> None:
        ...
        self._meters_per_pixel: float = float(
            self._runtime_config.get("meters_per_pixel", METERS_PER_PIXEL)
        )
        self._ref_bbox_area_at_1m: float = float(
            self._runtime_config.get("ref_bbox_area_at_1m", _REF_BBOX_AREA_AT_1M)
        )
        self._speed_scale: float = float(self._runtime_config.get("speed_scale", 1.0))
        self._distance_scale: float = float(self._runtime_config.get("distance_scale", 1.0))
        self._emergency_relative_margin_kmh: float = float(
            self._runtime_config.get(
                "emergency_relative_margin_kmh",
                _EMERGENCY_RELATIVE_MARGIN_KMH,
            )
        )
        self._emergency_speed_fallback_threshold: float = float(
            self._runtime_config.get(
                "emergency_speed_fallback_threshold",
                _EMERGENCY_SPEED_FALLBACK_THRESHOLD,
            )
        )
        self._distance_priority_enabled: bool = bool(
            self._runtime_config.get("distance_priority_enabled", True)
        )
        self._site_calibrated: bool = bool(
            self._runtime_config.get("site_calibrated", False)
        )
        self._per_class_speed_scale: dict[str, float] = self._load_scale_map(
            self._runtime_config.get("per_class_speed_scale", {})
        )
        self._per_class_distance_scale: dict[str, float] = self._load_scale_map(
            self._runtime_config.get("per_class_distance_scale", {})
        )
    ...
    @staticmethod
    def _load_scale_map(raw: object) -> dict[str, float]:
    ...
    def _load_runtime_config() -> dict:
        """Load optional runtime calibration / deployment settings from JSON."""
        cfg_path = os.environ.get(CALIBRATION_ENV_VAR, CALIBRATION_FILE_PATH)
        ...
        try:
            ...
            if isinstance(data, dict):
                logger.info("Loaded runtime calibration from %s.", cfg_path)
                return data
        except Exception as exc:
            logger.warning("Failed to load runtime calibration from %s: %s", cfg_path, exc)
        return {}

    def _resolve_model_path(self) -> Optional[str]:
        """Resolve the YOLO model path from env, config, or the default folder."""
        candidates = [
            os.environ.get(MODEL_PATH_ENV_VAR, ""),
            str(self._runtime_config.get("model_dir", "")),
    ...
    def _log_runtime_health(self) -> None:
        ...
        logger.info(
            "Runtime health: camera=%s gui=%s numpy=%s serial=%s tts=%s",
        ...
        if self._site_calibrated:
            logger.info("Runtime health: site calibration marked VERIFIED.")
        else:
            logger.warning(
                "Runtime health: site calibration is NOT verified. "
                "Fallback bbox speed/distance will be treated conservatively."
            )
    ...
    def start_camera_inference(self) -> None:
        ...
        def camera_loop() -> None:
            ...
            try:
                ...
                try:
                    if not hasattr(dai.node, "StereoDepth") or not hasattr(dai.node, "MonoCamera"):
                        raise RuntimeError("Stereo depth nodes are unavailable in this DepthAI build")

                    stereo = oak_pipeline.create(dai.node.StereoDepth)
                    preset_mode = getattr(dai.node.StereoDepth, "PresetMode", None)
                    if preset_mode is not None:
                        if hasattr(preset_mode, "HIGH_DENSITY"):
                            stereo.setDefaultProfilePreset(preset_mode.HIGH_DENSITY)
                        elif hasattr(preset_mode, "ROBOTICS"):
                            stereo.setDefaultProfilePreset(preset_mode.ROBOTICS)
                    if hasattr(stereo, "setLeftRightCheck"):
                        stereo.setLeftRightCheck(True)
                    if hasattr(stereo, "setSubpixel"):
                        stereo.setSubpixel(True)

                    board_socket = getattr(dai, "CameraBoardSocket", None)
                    left_socket = None
                    right_socket = None
                    align_socket = None
                    if board_socket is not None:
                        for name in ("LEFT", "CAM_B"):
                            if hasattr(board_socket, name):
                                left_socket = getattr(board_socket, name)
                                break
                        for name in ("RIGHT", "CAM_C"):
                            if hasattr(board_socket, name):
                                right_socket = getattr(board_socket, name)
                                break
                        for name in ("RGB", "CAM_A"):
                            if hasattr(board_socket, name):
                                align_socket = getattr(board_socket, name)
                                break

                    mono_left = oak_pipeline.create(dai.node.MonoCamera)
                    mono_right = oak_pipeline.create(dai.node.MonoCamera)
                    if left_socket is not None and hasattr(mono_left, "setBoardSocket"):
                        mono_left.setBoardSocket(left_socket)
                    elif hasattr(mono_left, "setCamera"):
                        mono_left.setCamera("left")
                    if right_socket is not None and hasattr(mono_right, "setBoardSocket"):
                        mono_right.setBoardSocket(right_socket)
                    elif hasattr(mono_right, "setCamera"):
                        mono_right.setCamera("right")

                    mono_props = getattr(dai, "MonoCameraProperties", None)
                    mono_res = getattr(mono_props, "SensorResolution", None) if mono_props is not None else None
                    if mono_res is not None:
                        for name in ("THE_400_P", "THE_480_P"):
                            if hasattr(mono_res, name):
                                if hasattr(mono_left, "setResolution"):
                                    mono_left.setResolution(getattr(mono_res, name))
                                if hasattr(mono_right, "setResolution"):
                                    mono_right.setResolution(getattr(mono_res, name))
                                break

                    mono_left.out.link(stereo.left)
                    mono_right.out.link(stereo.right)
                    if align_socket is not None and hasattr(stereo, "setDepthAlign"):
                        stereo.setDepthAlign(align_socket)

                    depth_q = stereo.depth.createOutputQueue()
                    logger.info("Camera inference: stereo depth enabled for distance estimation.")
                except Exception as exc:  # noqa: BLE001
                    depth_q = None
                    logger.warning(
                        "Camera inference: stereo depth unavailable, using bbox distance fallback: %s",
                        exc,
                    )
                oak_pipeline.start()
            ...
            while self._camera_running and oak_pipeline.isRunning():
                # Pull the latest frame from DepthAI (no preview window; avoid extra copy).
                ...
                if depth_q is not None:
                    try:
                        depth_pkt = depth_q.tryGet() if hasattr(depth_q, "tryGet") else None
                    except Exception:
                        depth_pkt = None
                    if depth_pkt is not None and hasattr(depth_pkt, "getFrame"):
                        last_depth_frame = depth_pkt.getFrame()
                self._on_inference_detections(detections, last_depth_frame, frame.shape[:2])

                # ── 7. Draw priority-coloured bounding boxes ───────────────
                # Rendering removed (no camera preview).
                # Live preview disabled: do not store/paint frames.
    ...
    def _on_inference_detections(
        self,
        detections: list,
        depth_frame: Optional[object] = None,
        ...
        for det in detections:
            ...
            candidates.append({
                ...
                "depth_distance_m": self._estimate_depth_distance_m(
                    depth_frame, bbox, frame_shape
                ),
            })
        ...
        for obj in candidates:
            ...
            if best_id is not None:
                ...
                t["bbox"]        = obj["bbox"]
                t["depth_distance_m"] = obj.get("depth_distance_m")
        ...
        for obj in unmatched:
            ...
            self._tracks[tid] = {
                ...
                "bbox":          obj["bbox"],
                "depth_distance_m": obj.get("depth_distance_m"),
                "smooth_speed":  0.0,
                "frames":        1,
                "first_seen_mono": now,
    ...
    def _compute_track_kinematics(self, track: dict) -> dict:
        ...
        dt    = l[4] - f[4]
        dsize = abs(l[3] - f[3])
        speed_scale = self._speed_scale * self._per_class_speed_scale.get(
            str(track.get("label", "")),
            1.0,
        )
        if dt >= _MIN_TIME_DELTA and dsize >= _MIN_SIZE_CHANGE:
            raw_speed = (dsize / dt) * self._meters_per_pixel * speed_scale * 3.6
        else:
            raw_speed = 0.0
        ...
        # Prefer OAK stereo depth when available; otherwise fall back to bbox area.
        depth_distance = track.get("depth_distance_m")
        if depth_distance is not None and float(depth_distance) > 0.0:
            distance = float(depth_distance)
            distance_confidence = 2
        else:
            latest_area = hist_list[-1][2]
            distance_scale = self._distance_scale * self._per_class_distance_scale.get(
                str(track.get("label", "")),
                1.0,
            )
            distance = self._estimate_distance(latest_area, distance_scale)
            distance_confidence = 1 if self._site_calibrated else 0
    ...
    def _estimate_distance(self, bbox_area: float, scale: float = 1.0) -> float:
        """Approximate distance (m) from bounding-box area."""
        if bbox_area <= 0:
            return 999.0
        return math.sqrt(self._ref_bbox_area_at_1m / bbox_area) * scale

    @staticmethod
    def _estimate_depth_distance_m(
        depth_frame: Optional[object],
        bbox: tuple[int, int, int, int],
        frame_shape: Optional[tuple[int, int]] = None,
    ) -> Optional[float]:
        """Return median OAK depth distance in metres for the bbox centre ROI."""
        if depth_frame is None or not _NUMPY_AVAILABLE:
            return None
        try:
            depth_arr = _np.asarray(depth_frame)
            if depth_arr.ndim < 2:
                return None

            src_h = int(frame_shape[0]) if frame_shape is not None else depth_arr.shape[0]
            src_w = int(frame_shape[1]) if frame_shape is not None else depth_arr.shape[1]
            depth_h, depth_w = depth_arr.shape[:2]
            if src_h <= 0 or src_w <= 0 or depth_h <= 0 or depth_w <= 0:
                return None

            xmin, ymin, xmax, ymax = bbox
            scale_x = depth_w / float(src_w)
            scale_y = depth_h / float(src_h)

            rx0 = max(int((xmin + (xmax - xmin) * 0.25) * scale_x), 0)
            ry0 = max(int((ymin + (ymax - ymin) * 0.25) * scale_y), 0)
            rx1 = min(int((xmax - (xmax - xmin) * 0.25) * scale_x), depth_w)
            ry1 = min(int((ymax - (ymax - ymin) * 0.25) * scale_y), depth_h)
            if rx1 <= rx0 or ry1 <= ry0:
                return None

            roi = depth_arr[ry0:ry1, rx0:rx1]
            if roi.size == 0:
                return None
            valid = roi[(roi > 0) & _np.isfinite(roi)]
            if valid.size == 0:
                return None

            return float(_np.median(valid)) / 1000.0
        except Exception:
            return None
    ...
    def _is_emergency_active(
        ...
        if avg_speed_kmh is not None and avg_speed_kmh > 0.0:
            ...
            fallback_threshold = self._emergency_speed_fallback_threshold
            if not self._site_calibrated:
                fallback_threshold += 10.0
            speed_crit = speed_kmh >= fallback_threshold
...
def _run_decision_tests() -> None:
    ...
    # Stereo depth helper: valid depth ROI beats bbox fallback when available.
    if _NUMPY_AVAILABLE:
        depth_arr = _np.full((480, 640), 3200, dtype=_np.uint16)
        depth_m = host._estimate_depth_distance_m(depth_arr, (100, 120, 180, 260), (480, 640))
        assert depth_m is not None and abs(depth_m - 3.2) < 0.01
    assert DISPLAY_WIDTH == 1280 and DISPLAY_HEIGHT == 720
